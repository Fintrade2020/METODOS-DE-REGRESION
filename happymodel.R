#En las próximas lineas se desarrolla una rutina de regresión a partir de una base de datos llamada "happy" que podemos encontrar en la libreria "faraway" 
#ANÁLISIS PRELIMINAR
#De la librería "faraway" llamamos base de datos "happy" (en ayuda puedes visualizar en qué consiste la base)
library(faraway)
data(happy)
help(happy)
#Presente la correlación y los gráficos de dispersión entre todas las variables.
attach(happy)
cor(happy)
plot(happy)
#REGRESIÓN LINEAL MÚLTIPLE
#Estime un modelo de regresión con todas las variables tomando "happy" como variable dependiente. Muestre los principales resultados. 
...
#Observe que la variable "love" es importante, sin embargo, se ha tomado como si fuera cuantitativa. Reconstruya esta variable mediante dos dummy y estime de nuevo el modelo donde las otras independientes serán "money" y "work". 
...
#Modifiquemos el análisis y supongamos que tenemos sex como dependiente.
...
piest<-ratio/(1+ratio)
plot(piest)
#La interpretación depende de los valores x. Por lo tanto podemos calcular el efecto marginal a partir del promedio de la probabilidad.
